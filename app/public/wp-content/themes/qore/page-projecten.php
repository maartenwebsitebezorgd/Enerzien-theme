...
<main class="main main-archive w-full" role="main">
    ...
    <?php if (!empty($query)): ?>
        <section class="posts bg-base-100 pt-8 md:pt-28 relative mb-10 target-viewport" id="projecten">
            ...
            <div class="container px-4 md:px-10">
                <?php if (!empty($categories)): ?>
                    <form method="get" action="<?php echo site_url() ?>/wp-admin/admin-ajax.php"
                        class="inline-flex w-full flex-row flex-wrap mb-8" id="post_filter">

                        <?php if (wp_is_mobile()): ?>
                            <select data-filter name="cat" id="filter"
                                class="inline-flex xl:hidden w-full shadow-lg border-none rounded-md h-14 pl-4">
                                <option selected value="all">
                                    <?php echo __('Alle projecten', 'qore'); ?>
                                </option>
                                <?php foreach ($categories as $cat) { ?>
                                    <option value="<?php echo $cat->slug; ?>" name="cat" id="<?php echo $cat->slug; ?>">
                                        <?php echo $cat->name; ?>
                                    </option>
                                <?php } ?>
                            </select>
                        <?php else: ?>
                            <div class="w-full flex-row flex-wrap hidden xl:inline-flex">
                                <div class="filter">
                                    <input checked data-filter="all" value="all" type="radio" name="cat" id="all"
                                        class="peer hidden" />
                                    <label for="all"
                                ...
                                <?php foreach ($categories as $cat) { ?>
                                    <div class="filter">
                                        <input data-filter="<?php echo $cat->slug; ?>" value="<?php echo $cat->slug; ?>" type="radio"
                                            name="cat" id="<?php echo $cat->slug; ?>" class="peer hidden" />
            ...
            <div class="container md:px-5">
                <div
                    class="grid grid-cols-1 sm:gap-x-5 sm:grid-cols-2 lg:grid-cols-3 text-base-content text-left md:min-h-[540px]">
